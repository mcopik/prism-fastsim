...
public class SynchronizedCommand implements CommandInterface
{
	...
	{
		throw new PrismException("Method getGuard is not "
				+ "defined for type SynchronizedCommand");
	}
	@Override
	public Update getUpdate() throws PrismException
	{
		throw new PrismException("Method getUpdate is not "
				+ "defined for type SynchronizedCommand");
	}
	@Override
	...
	public String toString()
	{
		...
		{
			builder.append(group.getKey()).append(" - sum: ").
				append(group.getValue().rate).append("\n");
			for(Pair<Command,Rate> cmd : group.getValue().cmds)
			{
				builder.append("Update sum: ").append(cmd.second).
						append("; ").append(cmd.first).append("\n");
			}
		}
